...
def submission(pred_array, submission_df, modelname, train_acc='none', desc='none', dest_dir='results/'):
    submission_df.iloc[:, 1] = pred_array

    now = dt.strftime(dt.now(), '%y-%m-%d')
    filename = f'{modelname}-train-{train_acc}-{desc}-{now}.csv'

    i = 0
    while os.path.exists(os.path.join(dest_dir, filename)):
        i += 1
        filename = f'{modelname}-train-{train_acc}-{desc}-{now}-{i}.csv'
